<template>
  <div class="review-search">
    ...
    <div class="search-box">
      <input type="text" v-model="searchId" placeholder="리뷰ID 입력" @keyup.enter="fetchReview">
      <button @click="fetchReview">검색</button>
    </div>
  ...
  methods: {
    async fetchReview() {
      ...
      if (!this.searchId.trim()) {
        this.errorMessage = '리뷰 ID를 입력해주세요.';
        return;
      }
      try {
        const url = `http://15.164.225.110:8080/review/${this.searchId}`;
        const response = await axios.get(url);
        this.review = response.data;
      } catch (error) {
        this.errorMessage = '리뷰를 찾을 수 없습니다. 리뷰 ID를 확인해주세요.';
      }
    },
    formatDate(date) {
